        ...
        <?php if ( get_the_title() ) { ?>
            ...
            <header class="entry-header">
                <?php the_title( '<h2 class="entry-title"><a href="' .
                    // esc_url( post_format_tools_url_grabber() ) //DEPRECATED (mjbrown)
                    esc_url( hybrid_get_the_post_format_url() )
                    . '" title="' . the_title_attribute( array( 'echo' => false ) ) . '">', '<span class="meta-nav">&rarr;</span></a></h2>' ); ?>
            </header><!-- .entry-header -->
